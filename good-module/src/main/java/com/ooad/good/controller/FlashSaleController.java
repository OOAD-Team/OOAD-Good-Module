...
import com.ooad.good.model.vo.flashSale.FlashSaleVo;
import com.ooad.good.model.vo.flashSale.FlashSkuVo;
...
public class FlashSaleController {
    ...
    public Flux<FlashSaleItemRetVo> getFlashSales(@Validated @PathVariable Long id){
        logger.debug("getFlashSalesById");
        return flashsaleService.getAllFlashsale(id);
    }
     /**
      * 平台管理员在某个时间段下新建秒杀
      * @param vo
    ...
    public Object insertFlashSale(@Validated @RequestBody FlashSaleVo vo, BindingResult bindingResult,
        ...
        if(!vo.getFlashDate().isAfter(LocalDateTime.now().plusDays(1))) {
            return new ReturnObject<>(ResponseCode.ACTIVITYALTER_INVALID);//.TOMORROW_FLASHSALE_INVALID);
        }

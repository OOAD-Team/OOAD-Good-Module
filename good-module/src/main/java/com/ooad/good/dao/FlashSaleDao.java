...
public class FlashSaleDao {
    ...
    public ReturnObject<FlashSaleItem> insertSku(FlashSaleItem flashSaleItem) {
        ...
        try {
            ...
            if (ret == 0) {
                ...
                retObj = new ReturnObject<>(ResponseCode.RESOURCE_ID_NOTEXIST, String.format("新增失败：" + flashPo.getId()));
            } else {
                //插入成功
    ...
    public ReturnObject deleteFlashsaleItem(Long fid, Long id) {
        ...
        try {
            ...
            if(ret!=0){logger.info("秒杀活动商品 id = " + id +"已被删除");
            retObj = new ReturnObject<>(ret);
            }
            else{logger.info("秒杀活动商品 id = " + id +"未被删除");
